...
const handleEmptyTrash = () => {
  ...
  const handleBulkRestore = () => {
    api
      ...
      .then(() => {
        setFiles(files.filter((f) => !selected.includes(f._id)));
        setSelected([]);
...
const handleBulkDelete = () => {
  if (window.confirm("Permanently delete selected files?")) {
    api
      .post("/trash/bulk/permanent-delete", { fileIds: selected })
      .then(() => {
    ...
    <div className="p-6">
      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {files.map((file) => (
            <div key={file._id} className="border p-4 rounded flex justify-between items-center">
